...
def prot_feats_seq(seq):
    ...
    tm = normalize(np.atleast_2d(tm), norm='l2',
                   copy=True, axis=1, return_norm=False)
    f.extend(tm[0])

    thm = np.array(threemerFromSeq(str(seq)))
    thm = normalize(np.atleast_2d(thm), norm='l2',
                    copy=True, axis=1, return_norm=False)
    f.extend(thm[0])
    return np.array(f)
...
def twomerFromSeq(s):
    ...
    # defines a vector of 343 length with zero entries
    ...
    try:
        ...
        for t in range(0, len(val)):  # find maximum value from val
            if m < val[t]:
                m = val[t]
                ind = t
        # m=group number of maximum occuring group alphabets in protein
        m = key[ind]
        for j in range(0, len(s)-k+1):
            kmer = s[j:j+k]
            c = ''
...
def analyzePhage(filename, output):
    ...
    # print(zip(ranks, scores, ids))
    # print(sorted(zip(ranks, scores, ids)))
    ...
    # print(X)
    model = pickle.load(open('model/svc.pickle', 'rb'))
    # print(model)
    labels = model.predict(X)
    # print(labels)
    labels, ids = zip(*sorted(zip(labels, ids)))
    res_fn = output+'.csv'
    with open(res_fn, 'w') as f:
        f.write('protein ID,Label\n')
...
def prot_feats_content(content):
    XX = []
    ids = []

    for rec in SeqIO.read(content, "fasta"):
        f = []
        X = ProteinAnalysis(str(rec.seq))
    # import pdb; pdb.set_trace()
        try:
...
def analyzePhage_1(filename):
    ...
    results = []
    for i in range(len(ids)):
        results.append([str(ids[i]),str(labels[i])])
    return results

def analyzePhageContent(content):
    X, ids = prot_feats_content(content)
    model = pickle.load(open('model/svc.pickle', 'rb'))
    # print(model)
    labels = model.predict(X)
    # print(labels)
